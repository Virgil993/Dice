...
export class Conversation {
  ...
  async create(token, users) {
    ...
    if (users.length < 2) {
      return {
        success: false,
        msg: "Not enough users to create a conversation",
      };
    }
  ...
  async getByBothUserId(user1Id, user2Id) {
    const conversation = await ConversationModel.find({
      users: { $all: [user2Id, user1Id] },
    });
    if (!conversation || conversation.length == 0) {
      return {
        success: false,
        msg: "error at get conversation by both user id",
      };
    }
  ...
  async addMessage(message) {
    try {
      const conversation = await ConversationModel.find({
        users: { $all: [message.recevier, message.sender] },
      });
