...
export class Message{
    ...
    async create(token, conversationId ,text, sender, recevier, date, seen){
        ...
        if(!text || !sender || !recevier || !date || !conversationId) {
            return {success: false, msg: "Missing required params at create message"}
        }
    ...
    async getByConversationId(token, conversationId){
        ...
        if(!conversationId){
            return {success: false, msg: "Missing required params at get messages from conversation"}
        }

        const elements = await MessageModel.find({conversationId: conversationId})
    ...
    async deleteAllByConversationId(token,conversationId){
        ...
        if(!conversationId){
            return {success: false, msg: "Missing required params at delete all messages by conversation id"}
        }
    ...
    async updateSeen(token,messages){
        try{
            ...
            console.log(err)
            return {success: false, msg:"error at update seen messages", error: err}
        }
    }
    ...
    async getNotifications(userId){
        try{
            ...
            const messages = await MessageModel.find({recevier:userId, seen:false})
            const conversations = []
            for(let i=0;i<messages.length;i++)
            {
            ...
            return {success: true, messages: messages, conversations: conversations}
        }
        catch(err){
            console.log(err)
